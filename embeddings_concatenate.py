...
def main():
    ...
    concatenate_embeddings_generate(embeddings_path=args.embeddings, out_path=args.output, vocab=vocab_from_path(args.vocabulary) if args.vocabulary else None, batch_size=args.batch_size, k=args.num_nearest_neighbor)


def concatenate_embeddings_generate(embeddings_path, out_path, vocab=None, batch_size=1024, k=10):
    ...
    for i_emb_path, emb_path in enumerate(embeddings_path):
        ...
        emb = load_embedding(emb_path, vocabulary=None, length_normalize=True, normalize_dimensionwise=False,
    ...
    for x, emb_path in enumerate(embeddings_path):
        ...
        for wi, w in enumerate(word_id):
            ...
            if wi % 1000 == 0:
                string = "<" + str(datetime.datetime.now()) + "> " + 'Calculating meta embeddind for embedding ' + str(x) + ': ' + str(
        ...
        with open('tmp/'+str(x), 'w') as file:
            for wi, w in enumerate(word_id):
                ...
                if wi % 1000 == 0:
                    string = "<" + str(datetime.datetime.now()) + "> " + 'Saving embedding ' + str(x) + ' to file : ' + str(
                        int(100 * wi / total_words)) + '%'
                    print(string, end="\r")
