...
def main():
    ...
    average_embeddings_generate(embeddings_path=args.embeddings, out_path=args.output, vocab=vocab_from_path(args.vocabulary) if args.vocabulary else None, batch_size=args.batch_size, k=args.num_nearest_neighbor)


def average_embeddings_generate(embeddings_path, out_path, vocab=None, batch_size=1024, k=10):
    ...
    if vocab is None:
        ...
        [print('Word ' + str(w) + ' not found in any embedding') for w in word_id - union]
    ...
    for i_voc, voc in enumerate(vocab_embeddings):
        ...
        print('We will generate ' + str(len(set(word_id) - voc)) + ' words for the embedding ' + str(i_voc))

    print()


    printTrace('Building matrix for word generation...')
    generation_vocab_matrix = [[x for x in range(len(embeddings_path))] for x in range(len(embeddings_path))]
    ...
    for i_emb_path, emb_path in enumerate(embeddings_path):
        ...
        emb = load_embedding(emb_path, vocabulary=None, lower=False, length_normalize=True, normalize_dimensionwise=False,
        ...
        for i_g, g in enumerate(generation_vocab_matrix[i_emb_path]):
            if len(g) > 0:
                ...
                interset_vocab = list(set.intersection(vocab_embeddings[i_emb_path], vocab_embeddings[i_g]))
                ...
                for i_batch, mb in enumerate(batch(m, batch_size)):
                    ...
                    string = "<" + str(datetime.datetime.now()) + ">  " + 'Using Embedding ' + str(
                        i_emb_path) + ' to generate vocab for Embedding ' + str(i_g) + ':  ' + str(
                        int(100 * (i_batch * batch_size) / total_words)) + '%'
                    print(string, end="\r")
                    ...
                    for i_result, indexes in enumerate(result):
                        nn_vocab[i_g][g[i_result + (batch_size * i_batch)]] = [interset_vocab[i] for i in indexes]
    ...
    for x, emb_path in enumerate(embeddings_path):
        ...
        for i, xb in enumerate(batch(word_id, batch_size)):
            ...
            string = "<" + str(datetime.datetime.now()) + ">  " + 'Embedding ' + str(x) + ': ' + str(
                int(100 * (i * batch_size) / total_words)) + '%'
            print(string, end="\r")
            m = []
            for w in xb:
            ...
            matrix[i * batch_size:i * batch_size + len(m)] = matrix_add(matrix[i * batch_size:i * batch_size + len(m)],
                                                                        m)
    ...
    with open(out_path, 'w+') as file:
        ...
        for wi, w in enumerate(word_id):
            ...
            if wi % 1000 == 0:
                string = "<" + str(datetime.datetime.now()) + "> " + 'Printing to file :' + str(
                    int(100 * wi / total_words)) + '%'
                print(string, end="\r")
